...
static constexpr size_t NUM_FIELDS_CAST_RELATION = 7;
...
inline bool parseLine(const std::string& line, TitleRelation& record) {
    ...
    // Iterate over all fields in the line.
    ...
    if (fieldIndex != NUM_FIELDS_TITLE_RELATION) {
        std::cerr << "NestedLoopUtils::parseLine(TitleRelation): Error: Too few fields in CSV line" << std::endl;
        return false;
    }
...
inline bool parseLine(const std::string& line, CastRelation& record) {
    ...
    if (fieldIndex != NUM_FIELDS_CAST_RELATION) {
        std::cerr << "NestedLoopUtils::parseLine(CastRelation): Error: Too few fields in CSV line" << std::endl;
        return false;
    }
...
std::vector<Relation> load(const std::string& filename, const size_t numberOfTuples = SIZE_MAX) {
    ...
    // Initialize filestream using filename.
    std::ifstream file(filename);

    if (!file.is_open()) {
        std::cerr << "NestedLoopJoin::load: Could not open file: " << filename << std::endl;
    }

    // Set up data vector that we load tuples from files into.
    std::vector<Relation> data;

    // Parse all lines from file and write data to vector using schemas of relations.
    std::string line;
    bool firstline = true;
    ...
    std::cout << "NestedLoopUtil::load: size of data " << data.size() << std::endl;
