...
static constexpr size_t NUM_FIELD_CAST_RELATION = 7;
	...
	inline bool operator<(const ResultRelation& lhs, const ResultRelation& rhs) {
		...
		int cmpResult;
		cmpResult = std::memcmp(lhs.title, rhs.title, sizeof(lhs.title));
		if (cmpResult != 0) return cmpResult < 0;
		cmpResult = std::memcmp(lhs.imdbIndex, rhs.imdbIndex, sizeof(lhs.imdbIndex));
		if (cmpResult != 0) return cmpResult < 0;
		...
		cmpResult = std::memcmp(lhs.phoneticCode, rhs.phoneticCode, sizeof(lhs.phoneticCode));
		if (cmpResult != 0) return cmpResult < 0;
		...
		cmpResult = std::memcmp(lhs.seriesYears, rhs.seriesYears, sizeof(lhs.seriesYears));
		if (cmpResult != 0) return cmpResult < 0;
		cmpResult = std::memcmp(lhs.md5sum, rhs.md5sum, sizeof(lhs.md5sum));
		if (cmpResult != 0) return cmpResult < 0;
		...
		cmpResult = std::memcmp(lhs.note, rhs.note, sizeof(lhs.note));
		if (cmpResult != 0) return cmpResult < 0;
